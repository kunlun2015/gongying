...
<div class="cate-change-select">
    <span class="active" data-type="1">订单</span>
                    ...
                    if(res.code === 0){
                        ...
                        if(type === 1){
                            $.each(res.data, function(index, value){
                                html = '';
                                html += '<li><div class="list-t"><div class="thumb"><img src="<?=Yii::$app->params['imgUrl']?>'+value.pictures[0]+'.thumb.jpg" alt="'+value.title+'"></div><div class="list-info"><span class="price action-btn"><a href="javascript:;" class="delete" data-id="'+value.id+'">删除</a></span><p class="title">'+value.title+'</p><p class="num">数量：'+value.num+'</p><p class="area">交付地区：'+value.delivery_area+'</p></div></div><div class="attr"><div class="cate"><span>'+value.fname+'</span><span>'+value.sname+'</span><span>'+value.tname+'</span></div><div class="keywords"><a href="">#'+value.fname+'#</a></div></div></li>';
                                $('.list').eq(type-1).append(html);
                            })
                        }else if(type === 2){
                            $.each(res.data, function(index, value){
                                html = '';
                                html += '<li><div class="list-t"><div class="thumb"><img src="<?=Yii::$app->params['imgUrl']?>'+value.pictures[0]+'.thumb.jpg" alt="'+value.title+'"></div><div class="list-info"><span class="price action-btn"><a href="javascript:;" class="delete" data-id="'+value.id+'">删除</a></span><p class="title">'+value.title+'</p><p class="num">服务商：'+value.num+'</p><p class="area">所在地区：'+value.delivery_area+'</p></div></div><div class="attr"><div class="cate"><span>'+value.fname+'</span><span>'+value.sname+'</span><span>'+value.tname+'</span></div><div class="keywords"><a href="">#'+value.fname+'#</a></div></div></li>';
                                $('.list').eq(type-1).append(html);
                            })
                        }                        
